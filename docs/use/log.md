...
`.log halt` / `.log end [<日志名>]`

`.log halt` 强制停止当前日志而不上传。`.log end` 停止并上传当前日志；指定日志名时，可以结束并上传该命名日志，即使它不是当前正在记录的日志，也不会影响另一个当前日志。
...
`.log get <日志名> -f` 强制重新上传，而不是复用已有链接。

`.log list` 会为日志名显示一个短哈希别名。日志名过长、包含特殊字符或不便输入时，`.log stat`、`.log get`、`.log end` 和 `.log export` 等操作都可以使用这个别名代替完整名称。

例如，`.log list` 的输出如下：

```text
正在列出存在于此群的记录:
【072ffabd】: 2026_05_21_19_13_25
【784166a0】: 咪
【0b8f97d6】: test1
```

其中方括号内的内容是日志的短哈希别名，例如 `072ffabd`。

::: warning

低于 `v1.6.0` 的版本中，过长或包含特殊字符的日志名可能导致海豹核心崩溃。请迁移至较新版本。
...
::: info 修改群名片需要骰子账号有适当权限 <Badge type="tip" text="v1.6.0" />

对于能够查询群身份的平台，执行 `.sn` 的非帮助操作前会检查骰子账号是否为群主或群管理员；权限不足时不会继续修改。协议端不支持角色查询时，海豹无法预先确认权限，会跳过这项检查并直接向协议端发送修改群名片的请求；是否成功以及返回的提示均以平台实际处理结果为准。
