...
### 理智损失上限与减半 <Badge type="tip" text="v1.6.0"/>

从 <Badge type="tip" text="v1.6.0"/> 起，海豹支持指定理智检定的损失上限和/或减半，分别对应「习惯恐怖」规则和「神话淬炼」规则（旧译「精神固化」）。
...
骰主可以在 WebUI 的「综合设置 - 基本设置」中开启“制卡使用合并转发”。开启后，多张制卡结果会优先以合并转发发送；当前平台或协议端不支持合并转发时，会自动回退为普通文本。
