...
进行检定，并且使用指定个数(如果不指定，为1个)的奖励骰(`b`)或惩罚骰(`p`)。
...
随机生成指定数量的调查员属性。如果省略数量，将生成1个。生成的属性包括8项基础属性与幸运，同时会计算HP点数。

此外，也会计算属性的总值供用户参考。总值包括两个，其一是8项基础属性的总值，其二是前者与幸运的总值。
