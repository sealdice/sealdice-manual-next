...
在海豹指令中 ID 有两种格式，如 `user 123456` 和 `QQ:12345`，前者使用于同一平台，后者可跨平台使用，如在 WebUI 的**指令测试**中请使用统一 ID。

统一 ID 示例：QQ:12345、QQ-Group:12345
...
`.find (#<分组名>) <关键字> (--page=<页码>)` 用关键字进行词条查询；如果提供了分组名，则只在指定分组中进行查询。可指定页码，不指定为第一页，一页显示 4 条结果。
...
`.find <关键字> --num=10` 单页显示 10 条结果，需要更多结果可使用。
