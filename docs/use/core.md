...
此指令用于管理最高权限。Master 列表为空时，任何人都可以使用 `.master add me` 成为第一位 Master；一旦列表中存在实际 Master，后续管理操作就只允许 Master 执行。

Master 可以是用户统一 ID，也可以是群组统一 ID。将群组加入 Master 列表会使该群内所有人都获得 Master 权限，应谨慎使用。群主、群管理员和邀请者不会因此自动成为 Master，它们只在部分群管理指令中拥有较低级别的操作权限。

`.master add me` 为指令发送者添加 Master 权限（仅没有添加过 Master 时可用）。

`.master add @A [@B ...]` 为指定用户添加 Master 权限。

`.master del @A [@B ...]` 移除指定用户的 Master 权限。

`.master list` 查看当前 Master 权限列表。

`.master relogin` 30 秒后重新登录；使用 `--now` 立即执行，使用 `--cancel` 取消等待中的重登录。该操作有 5 分钟冷却，也可能使骰子失联。
...
`.master jsclear <插件 ID>` 清除指定 JavaScript 插件的存储，并重载 JavaScript 环境。该操作会删除插件数据，执行前应确认插件 ID。
...
`.userid` 查看自己的统一 ID；在群聊中还会同时显示当前群组的统一 ID。私聊中只显示个人 ID。

统一 ID 用于 Master、黑白名单、通知列表和跨平台群组管理。不要自行拼接 ID，应优先复制此指令的输出。
...
- `.bot on` 在当前群启用骰子。
- `.bot off` 在当前群停用骰子；已开启的跑团日志也会暂停记录。
- `.bot bye`、`.bot exit`、`.bot quit` 请求退出当前群。

## `.dismiss` 退出当前群 <Badge type="tip" text="v1.6.0" />

`.dismiss` 是专用退群指令，作用与 `.bot bye` 相同，只能在群聊中使用。默认必须 `@` 骰子后执行；只有骰主明确开启“退群指令无需 @”后才接受裸指令。

在 QQ、OneBot 11 和 Milky 等能够查询群身份的平台上，如果骰子是群主，退出会导致群聊被解散。此时海豹会返回临时 4 位确认码，必须在同一群中按提示重新输入完整命令。无法确认骰子群身份时也会进入同样的安全确认流程。

::: danger 群主退群

确认码验证通过后，群主身份的骰子会解散群聊，而不仅是离开。不要转发确认命令，也不要在不清楚骰子群身份时执行。
...
`.ext all on` 开启当前群中所有未启用的扩展；`.ext all off` 关闭当前群中所有已启用的扩展。批量开启时如果检测到扩展冲突，海豹会列出冲突项；同名指令通常由后开启的扩展优先处理。

实际扩展数量取决于当前版本、JavaScript 插件和已安装的扩展包，请以 `.ext list` 输出为准。
