...
在海豹指令中ID有两种格式，如`user 123456`和`QQ:12345`，前者使用于同一平台，后者可跨平台使用，如在 WebUI 的**指令测试**中请使用统一 ID 。
...
`.ban add user <帐号> (<原因>)` 将用户拉入黑名单，原因可不填。

`.ban add group <群号> (<原因>)` 将群聊拉入黑名单，原因可不填。

`.ban add <统一ID> (<原因>)` 使用统一 ID，其他同上。

`.ban trust <统一ID>` 仅可使用统一 ID,将用户添加至信任列表。

`.ban rm user <帐号>` 将用户移出黑名单/信任列表。

`.ban rm group <群号>` 将群聊移出黑名单/信任列表。

`.ban rm <统一ID>` 使用统一 ID，其他同上。
...
`.find <关键字> --num=10` 单页显示10条结果，需要更多结果可使用。
