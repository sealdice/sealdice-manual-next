...
`.dnd (<数量>)` 随机生成指定数量的角色属性。如果省略数量将生成 1 组。生成的属性为 6 项数值，可以自由分配。

`.dndx (<数量>)` 类似地，但是 6 个数值已经指定到 6 个属性。
...
`.ri <先攻值> (<角色名>)` 为指定角色设定先攻值。如果省略角色名，为自己的角色，以下的角色名部分相同。
...
`.ss init 4 3 2` 设置 1 2 3 环的法术位上限，以此类推到 9 环。
