...
你也可添加群组 ID，在指定的群中所有人均视为有 Master 权限。Master 列表为空时，任何人都能通过 `.master add me` 取得首位 Master；配置完成后应确认列表中至少保留一个可信用户，并妥善保存 WebUI 解锁码。
...
从 <Badge type="tip" text="v1.6.0" /> 起，可以开启“退出不活跃群组通知汇总模式”。开启后不再为每个群分别发送通知，而是在任务开始和结束时发送简报，汇总已处理、已跳过、已取消和已开始退出的数量。大规模清理群组时建议开启。
