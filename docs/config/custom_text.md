...
`制卡_自由分配模式` 用于 `.dnd`，`制卡_预设模式` 用于带属性名的 `.dndx`。两者都通过 `{$t制卡结果文本}` 插入生成的属性。

一次生成多组属性时，`制卡_分隔符` 用于分隔每组结果。

#### `.rc` 检定

单轮检定使用 `检定`。多轮检定会先为每一轮调用 `检定_单项结果文本`，再将全部结果写入 `{$t结果文本}`，最后调用 `检定_多轮`。请求的轮数超过上限时使用 `检定_轮数过多警告`。

这些文案可以使用以下变量：

- `{$t骰子出目}`：本轮 D20 的基础出目；使用优势或劣势时为最终选取的 D20 结果；
- `{$t检定过程文本}`：基础骰与属性、调整值的计算过程；
- `{$t检定结果}`：基础骰加上属性和调整值后的最终结果；
- `{$t次数}`：多轮检定的轮数；
- `{$t结果文本}`：由各轮 `检定_单项结果文本` 拼接出的完整结果。
