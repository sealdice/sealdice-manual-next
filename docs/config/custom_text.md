...
当你在新版本海豹导入旧版本文案时，需要重启海豹核心以免出现缺少文案的情况。
...
`检定_单项结果文本`中的 `{$t判定结果}` 即调用 `判定-常规` 对应的回复词，当你的鉴定为多轮鉴定时，将调用 `判定-简短`。

对于`判定_必须_困难_失败`此类必须判定的回复词，使用于指定鉴定难度的情况，如`.ra 困难侦察`。
...
`技能成长_批量`中的 `{$t总结果文本}`  即 n 次调用`技能成长_批量_单条`，`技能成长_批量_分隔符`为每一单条中间的分隔符。
