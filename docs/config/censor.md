...
- 发送警告：会向触发用户发送警告信息；
- 发送编码后的命中详情：向当前会话发送命中等级、命中词和上下文片段；命中词与上下文使用 Base64 编码，避免敏感内容直接出现在聊天记录中；
- 通知 Master：会通知骰主；
...
在「拦截日志」标签页，你可以看到在何时、何地、哪个用户触发了什么等级的敏感词，具体的内容是什么。多个命中词会按稳定顺序记录，便于比较重复事件。

“发送编码后的命中详情”只会截取命中位置附近的上下文，并将命中词和上下文分别编码。Base64 不是加密，拿到内容的人可以解码，因此不要把详情发送到不可信的群聊。
