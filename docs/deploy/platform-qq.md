...
从 <Badge type="tip" text="v1.4.5" /> 开始，我们推荐使用 [内置客户端](#内置客户端) 进行连接，这是面向一般用户提供的简单对接方式，对于有能力的骰主，我们更推荐其他分离方案。
...
::: warning 注意：对接 PC 端 QQNT 协议时对 `戳一戳` 功能的适配

由于内置客户端、旧版本 Lagrange、旧版本 LLOneBot 和旧版本 Napcat**并未完全实现**戳一戳功能。请使用上述客户端连接 QQ 的用户请注意自己的框架版本是否支持戳一戳功能，若不支持请及时更新或关闭海豹核心后台位于 `综合设置` - `基本设置` 的 `启用戳一戳` 开关，以免产生不必要的报错和麻烦。
...
请按照 [NapCat 官方手册](https://napneko.github.io/zh-CN/guide/getting-started)下载安装，如果你清楚你的技术能力不足，请不要安装 9.9.12 版本 QQ。
