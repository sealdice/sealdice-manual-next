...
由于大部分用户并不擅长非图形化页面的操作，此处提供一个 GUI 工具供使用，本教程选用 squirrelsql 作为图形化页面的使用。本教程主要服务于 Windows Server 用户，其他用户可自行按照原理使用 SQL 命令来创建等效的数据库。

在 这里 下载定制好的 squirrelsql。该 squirrel sql 设置为开箱即用，允许修改 SQLite/MySQL/PostgreSQL 三种类型的数据库。请将 squirrel sql 放置在和 MySQL 相同的服务器上。
