...
`types/seal.d.ts` 文件中维护了海豹提供的 API。如果你发现运行时已有接口但类型文件尚未收录，可以先在项目内补充类型来解决报错，并向模板仓库提交修正。

类型声明不是运行时兼容保证。升级核心 API 或发布插件前，应同步模板仓库中的最新版 `seal.d.ts`，并在 `header.txt` 更新 `@sealVersion`。

### 工程化插件可以放入扩展包吗？

可以。工程仍编译出 JS 文件，再由扩展包清单把它作为 `script` 内容收录。规则模板、牌堆、回复和帮助文档也可以放在同一扩展包中；打包格式、依赖和权限见[扩展包与商店](../config/package.md)。
