...
某些情况下，我们希望能够提供一个实时更新内容的牌堆，骰主不用对牌堆进行更新，用户也能抽取牌堆的最新内容。（例如提供一个公骰列表牌堆等）

海豹支持为牌堆提供云端内容，这需要牌堆作者有能力提供一个 API 接口，**每次** 配置了接口的牌组被抽取的时候，都会调用该接口获取该牌组的最新数据。

#### API 接口要求

海豹将以 HTTP GET 请求的方式请求对应接口，接口应当返回一个 JSON 字符串数组数据，返回结果示例如下：
...
正如上面示例中看到的那样，在 TOML 格式的复杂牌组中，有一些云端内容的相关配置项：

- `cloud_extra`：为 `true` 时表示这个牌组将提供云端内容，每个牌组都可以单独控制云端相关设置；
- `options_urls`：云端内容的 API 链接，可以看出链接能够配置多个。**每次抽取这个牌组时，都会依次调用 API，有一个成功返回即作为结果**；
- `distinct`：抽取时获取到云端内容后，将和这个牌组本身配置了的 `options` 进行合并，将在合并后内容中抽取结果。这一选项为 `true` 时，云端内容与 `options` 合并后，将去重后再抽取结果。
