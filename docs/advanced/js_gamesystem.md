...
## 新版本补充 <Badge type="tip" text="LatestVersion"/>
...
从 <Badge type="tip" text="LatestVersion"/> 起，DiceScript 上下文提供 `actor` 角色卡对象。规则指令可通过 `seal.format(ctx, '{actor.属性名}')` 读取角色属性；属性别名会按当前规则模板归一化。`actor` 属于 DiceScript/RollVM，不是普通 JavaScript 全局对象，完整说明见 [API 列表](./js_api_list.md#角色卡-actor-对象)。
